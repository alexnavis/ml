...
function evaluate({ mlmodel, configuration, state, scoreanalysis = null }, /*configuration, state, machinelearning*/) {
  try {
    const random_forest = mlmodel[ 'random_forest' ];
    ...
    const model_config = JSON.parse(random_forest.model);
    const classifier = RFClassifier.load(model_config);
    let prediction = (mlmodel.type === 'binary') 
      ? helpers.runRandomForestPrediction.call(classifier, cleaned) 
      : helpers.runRandomForestPredictionCategorical.call(classifier, cleaned);
    ...
    datasource_headers.forEach((header, i) => {
      ...
      let explainability_result = (model_type === 'binary') 
        ? helpers.runRandomForestPrediction.call(classifier, [new_cleaned]) 
        : helpers.runRandomForestPredictionCategorical.call(classifier, [new_cleaned]);
